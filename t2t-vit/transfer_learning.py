...
testloader = torch.utils.data.DataLoader(
        testset, batch_size=args.b, shuffle=False, num_workers=8)
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        train_writer.add_scalar('train_loss', loss.item(), global_step=int(epoch * len(trainloader) + batch_idx))

        number_batch = targets.size()[0]
        _, predicted = torch.max(outputs.data, dim=1)
        correct_batch = (predicted == targets).sum().item()
        accuracy_batch = 100 * correct_batch / number_batch

        train_writer.add_scalar('train_acc', accuracy_batch, global_step=int(epoch * len(trainloader) + batch_idx))
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(testloader):
            ...
            test_writer.add_scalar('test_loss', loss.item(), global_step=int(epoch * len(testloader) + batch_idx))
            ...
            test_writer.add_scalar('test_acc', accuracy_batch, global_step=int(epoch * len(testloader) + batch_idx))
